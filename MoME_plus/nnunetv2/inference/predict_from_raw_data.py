...
class nnUNetPredictor(object):
    def __init__(self,
                 ...
                 allow_tqdm: bool = True,MultiMod=[1,0,0,0],csv_path='demo.csv'):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        for i, f in enumerate(use_folds):
            f = int(f) if f in ['0','1','2','3'] else str(f)
            checkpoint = torch.load(join(model_training_output_dir, f'fold_{f}', checkpoint_name),
                                    map_location=torch.device('cpu'))
            checkpoint1 = torch.load(join(model_training_output_dir, f'fold_{f}', checkpoint_name.replace('best','best1')),
                                    map_location=torch.device('cpu'))
            checkpoint2 = torch.load(join(model_training_output_dir, f'fold_{f}', checkpoint_name.replace('best','best2')),
                                    map_location=torch.device('cpu'))
            checkpoint3 = torch.load(join(model_training_output_dir, f'fold_{f}', checkpoint_name.replace('best','best3')),
                                    map_location=torch.device('cpu'))
            checkpoint4 = torch.load(join(model_training_output_dir, f'fold_{f}', checkpoint_name.replace('best','best4')),
                                    map_location=torch.device('cpu'))
            checkpoint_dispatch = torch.load(join(model_training_output_dir, f'fold_{f}', checkpoint_name.replace('best','best_dispatch')),
                                    map_location=torch.device('cpu'))
            # checkpoint5 = torch.load(join(model_training_output_dir, f'fold_{f}', checkpoint_name.replace('best','best5')),
            #                         map_location=torch.device('cpu'))
            ...
            parameters1.append(checkpoint1['network_weights'])
            parameters2.append(checkpoint2['network_weights'])
            parameters3.append(checkpoint3['network_weights'])
            parameters4.append(checkpoint4['network_weights'])
            parameters_dispatch.append(checkpoint_dispatch['network_weights'])
            # parameters5.append(checkpoint5['network_weights'])
        ...
        network = trainer_class.build_network_architecture(plans_manager, dataset_json, configuration_manager,
                                                           num_experts+output_channels*num_experts, enable_deep_supervision=False,num_experts=num_experts,Mod_Prior=True)
        network1 = trainer_class.build_network_architecture(plans_manager, dataset_json, configuration_manager,
                                                           num_input_channels, enable_deep_supervision=False)
        network2 = trainer_class.build_network_architecture(plans_manager, dataset_json, configuration_manager,
                                                           num_input_channels, enable_deep_supervision=False)
        network3 = trainer_class.build_network_architecture(plans_manager, dataset_json, configuration_manager,
                                                           num_input_channels, enable_deep_supervision=False)
        network4 = trainer_class.build_network_architecture(plans_manager, dataset_json, configuration_manager,
                                                           num_input_channels, enable_deep_supervision=False)
        # network5 = trainer_class.build_network_architecture(plans_manager, dataset_json, configuration_manager,
        #                                                    num_input_channels, enable_deep_supervision=False)
        dispatch_network = ClsDispatchNet(input_dim=num_experts)
        ...
        self.list_of_parameters1 = parameters1
        self.list_of_parameters2 = parameters2
        self.list_of_parameters3 = parameters3
        self.list_of_parameters4 = parameters4
        self.list_of_parameters_dispatch = parameters_dispatch
        # self.list_of_parameters5 = parameters5
        
        self.network = network
        self.network1:nn.Module = network1
        self.network2:nn.Module = network2
        self.network3:nn.Module = network3
        self.network4:nn.Module = network4
        self.dispatch_network:nn.Module = dispatch_network
        # self.network5:nn.Module = network5
    ...
    def predict_from_files(self,
                           ...
                           output_folder_or_list_of_truncated_output_files: Union[str, None, List[str]], csv_path:str,
                           save_probabilities: bool = False,
                           overwrite: bool = True,
    ...
    def _internal_get_data_iterator_from_lists_of_filenames(self,
        ...
        return preprocessing_iterator_fromfiles(input_list_of_lists, seg_from_prev_stage_files,
                                                ...
                                                self.verbose_preprocessing,self.MultiMod)
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            if self.perform_everything_on_gpu:
                try:
                    for params,params1,params2,params3,params4,param_dispatch in zip(self.list_of_parameters,self.list_of_parameters1,self.list_of_parameters2,self.list_of_parameters3,self.list_of_parameters4,self.list_of_parameters_dispatch):

                        # messing with state dict names...
                        if not isinstance(self.network, OptimizedModule):
                            self.network.load_state_dict(params)
                            self.network1.load_state_dict(params1)
                            self.network2.load_state_dict(params2)
                            self.network3.load_state_dict(params3)
                            self.network4.load_state_dict(params4)
                            self.dispatch_network.load_state_dict(param_dispatch)
                            # self.network5.load_state_dict(params5)
                        ...
                        if prediction is None:
                            ...
                            prediction += self.predict_sliding_window_return_logits(data)
                    # for param in self.list_of_parameters_dispatch:
                    #     self.dispatch_network.load_state_dict(param)
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor,flag:int) -> torch.Tensor:
        mirror_axes = self.allowed_mirroring_axes if self.use_mirroring else None

        sup_losses = self.MultiMod
        mask_modality = torch.tensor(sup_losses).to(self.device)*1.0 # batchsize =1 in inference but batchsize =2 in training
        mask_modality = mask_modality.unsqueeze(0)
        modality_embading = mask_modality[...,None].repeat(1,1,4).permute(0,2,1) # M(b,4,4)
        # data = x*mask_modality.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) #
        # for b in range(x.shape[0]):
        #     for c in range(x.shape[1]):
        #         if mask_modality[c] == 0:
        #             x[b,c] = torch.randn(x.shape[2:], device='cuda')
        
        raw_dispatch_embading = self.dispatch_network(mask_modality)  # Assuming dispatch_network returns a tensor of shape (b=2, 4, 4)
        min_values, _ = torch.min(raw_dispatch_embading, dim=2, keepdim=True)
        max_values, _ = torch.max(raw_dispatch_embading, dim=2, keepdim=True)
        dispatch_embading = (raw_dispatch_embading - min_values + 1e-4) / (max_values - min_values + 1e-4)
        '''
        OptionONE------Prior Constrain-TOPK: 1.mask 2.avaliable channel
        '''     
        sparse_mask_dispatch_embading = dispatch_embading*modality_embading.to(self.device) #(2,4,128,128,128)
        B, M = sparse_mask_dispatch_embading.shape[:2]
        c_mask = torch.eye(M)[None,...].repeat(B,1,1).to(sparse_mask_dispatch_embading)
        keep_modality = mask_modality == 1
        sparse_mask_dispatch_embading[keep_modality,:] = c_mask[keep_modality,:]  # mask_dispatch_embading (argmx) shape(2,4)
        dispatched_data = torch.sum(sparse_mask_dispatch_embading[...,None, None, None] * x.unsqueeze(1), dim=2)
        
        x = dispatched_data
        if flag == 0:
            with open(self.csv_path, 'a', newline='') as file:
                writer= csv.writer(file)
                # writer.writerow(mask_dispatch_embading.tolist()[0])
                print(sparse_mask_dispatch_embading)
                writer.writerow(sparse_mask_dispatch_embading.tolist()[0])
                # writer.writerow([*mask_dispatch_embading[1]])
            
        def MOE_predict(x):           
            Feature1,prediction1 = self.network1(x[:,0].unsqueeze(1))
            Feature2,prediction2 = self.network2(x[:,1].unsqueeze(1))
            Feature3,prediction3 = self.network3(x[:,2].unsqueeze(1))
            Feature4,prediction4 = self.network4(x[:,3].unsqueeze(1))
            # Feature5,prediction5 = self.network5(x)
            mask_modality = (torch.tensor(self.MultiMod).to(self.device)*1.0).unsqueeze(0)
            coutput = torch.cat((x,Feature1[0], Feature2[0],Feature3[0], Feature4[0]), dim=1)  #(2,1+32+32,128,128,128)
            # coutput_debug = torch.cat((output1[0], output2[0]), dim=1)
            _,output = self.network([coutput,mask_modality])
            classnum = 4
            prediction = prediction1*output[:,:classnum,:,:,:]+prediction2*output[:,classnum:2*classnum,:,:,:]+prediction3*output[:,2*classnum:3*classnum,:,:,:] \
                +prediction4*output[:,3*classnum:4*classnum,:,:,:]
            return prediction
        prediction = MOE_predict(x)
        if mirror_axes is not None:
            ...
            if 0 in mirror_axes:
                prediction += torch.flip(MOE_predict(torch.flip(x, (2,))), (2,))
            if 1 in mirror_axes:
                prediction += torch.flip(MOE_predict(torch.flip(x, (3,))), (3,))
            if 2 in mirror_axes:
                prediction += torch.flip(MOE_predict(torch.flip(x, (4,))), (4,))
            if 0 in mirror_axes and 1 in mirror_axes:
                prediction += torch.flip(MOE_predict(torch.flip(x, (2, 3))), (2, 3))
            if 0 in mirror_axes and 2 in mirror_axes:
                prediction += torch.flip(MOE_predict(torch.flip(x, (2, 4))), (2, 4))
            if 1 in mirror_axes and 2 in mirror_axes:
                prediction += torch.flip(MOE_predict(torch.flip(x, (3, 4))), (3, 4))
            if 0 in mirror_axes and 1 in mirror_axes and 2 in mirror_axes:
                prediction += torch.flip(MOE_predict(torch.flip(x, (2, 3, 4))), (2, 3, 4))
    ...
    def predict_sliding_window_return_logits(self, input_image: torch.Tensor) \
        ...
        self.network3.eval()
        self.network4 = self.network4.to(self.device)
        self.network4.eval()
        self.dispatch_network = self.dispatch_network.to(self.device)
        self.dispatch_network.eval()
        # self.network5 = self.network5.to(self.device)
        # self.network5.eval()
        ...
        with torch.no_grad():
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                ...
                for sl in tqdm(slicers, disable=not self.allow_tqdm):
                    ...
                    prediction = self._internal_maybe_mirror_and_predict(workon,flag)[0].to(results_device)
                    flag = 2
...
def predict_entry_point_modelfolder():
    ...
    parser.add_argument('--MultiMod', nargs='+', type=int, default=[1,0,0,0],
                    help='A list, eg: --MultiMod 1 0 0 0')
...
def predict_entry_point():
    ...
    args.f = [str(i) if 1  else int(i) for i in args.f]
    ...
    predictor = nnUNetPredictor(tile_step_size=args.step_size,
                                ...
                                verbose_preprocessing=False,MultiMod=args.MultiMod,csv_path=csv_path)
